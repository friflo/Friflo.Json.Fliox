    ...
    public partial class TestHappy
    {
        ...
        [Test] public static void TestAuthRights () {
            using (var _                = SharedEnv.Default) // for LeakTestsFixture
            {
                SingleThreadSynchronizationContext.Run(async () => {
                    using (var userDatabase     = new FileDatabase(TestGlobals.UserDB, CommonUtils.GetBasePath() + "assets~/DB/user_db", new UserDBHandler()))
        ...
        private static async Task AssertAuthAccessSubscriptions(FlioxHub hub) {
            using (var mutateUser       = new PocStore(hub) { UserId = "test-deny", ClientId = "auth-1" }) {
                ...
                var articleChanges = mutateUser.articles.SubscribeChanges(EntityChange.upsert, (changes, context) => { });
                await mutateUser.TrySyncTasks();
                AreEqual("PermissionDenied ~ not authorized. user: test-deny", articleChanges.Error.Message);
                
                var articleDeletes = mutateUser.articles.SubscribeChanges(EntityChange.delete, (changes, context) => { });
                await mutateUser.TrySyncTasks();
