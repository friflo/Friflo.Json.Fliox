    ...
    public class TestSelect : LeakTestsFixture
    {
        ...
        public void TestObjectSelect() {
            ...
            using (var objectSelector   = new MemberAccessor(typeStore))
            {
                ...
                var json    = new JsonUtf8(jsonWriter.WriteAsArray(sample));
                var selectors = new[] {
                    ".childStructNull1",
        ...
        public void TestArraySelect() {
            ...
            using (var jsonSelector     = new JsonSelector())
            {
                var store = new Store();
                store.InitSample();
                var json        = new JsonUtf8(jsonWriter.WriteAsArray(store));
                var selectors   = new[] {
                    ".books[*].title",
                    ".books[*].author",
                    ".books[*].chapters[*].name",
        ...
        public void TestNoAlloc() {
            ...
            {
                jsonMapper.Pretty = true;
                var peter  = new JsonUtf8(jsonMapper.WriteAsArray(TestQuery.Peter));
