    ...
    public class TestInstanceFactory
    {
        ...
        private void TestInterface(TypeAccess typeAccess) {
            ...
            using (var writer = new ObjectWriter(typeStore))
            {
                ...
                AreEqual("type requires instantiatable types by [Fri.Instance()] or [Fri.Polymorph()] on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+IVehicle", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<Abstract>("{}"));
                AreEqual("type requires instantiatable types by [Fri.Instance()] or [Fri.Polymorph()] on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+Abstract", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<ITestIncompatibleInstance>("{}"));
                AreEqual("[Fri.Instance(Book)] type must extend annotated type: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+ITestIncompatibleInstance", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<ITestInstanceNull>("{}"));
                AreEqual("[Fri.Instance(null)] type must not be null on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+ITestInstanceNull", e.Message);
            }
        }
        ...
        private void TestPolymorphic(TypeAccess typeAccess) {
            ...
            using (var writer = new ObjectWriter(typeStore))
            {
                ...
                StringAssert.Contains("No [Fri.Polymorph] type declared for discriminant: 'Tiger' on type: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+IAnimal", reader.Error.msg.ToString());
                
                reader.Read<IAnimal>("{}");
                StringAssert.Contains("Expect discriminator \"animalType\": \"...\" as first JSON member for type: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+IAnimal", reader.Error.msg.ToString());
                
                var e = Throws<InvalidOperationException>(() => reader.Read<TestIncompatiblePolymorph>("{}"));
                AreEqual("[Fri.Polymorph(Book)] type must extend annotated type: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+TestIncompatiblePolymorph", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<TestPolymorphNull>("{}"));
                AreEqual("[Fri.Polymorph(null)] type must not be null on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+TestPolymorphNull", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<TestNoDiscriminator>("{}"));
                AreEqual("specified [Fri.Polymorph] attribute require [Fri.Discriminator] on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+TestNoDiscriminator", e.Message);
                
                e = Throws<InvalidOperationException>(() => reader.Read<TestNoPolymorph>("{}"));
                AreEqual("specified [Fri.Discriminator] require at least one [Fri.Polymorph] attribute on: Friflo.Json.Tests.Common.UnitTest.Flow.Mapper.TestInstanceFactory+TestNoPolymorph", e.Message);
            }
        }
