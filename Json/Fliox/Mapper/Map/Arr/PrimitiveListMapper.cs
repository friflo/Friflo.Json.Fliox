...
namespace Friflo.Json.Fliox.Mapper.Map.Arr
{
    ...
    internal sealed class PrimitiveListMapper<T> : CollectionMapper<List<T>, T>
    {
        ...
        public override DiffType Diff(Differ differ, List<T> left, List<T> right) {
        ...
        public override List<T> Read(ref Reader reader, List<T> slot, out bool success) {
            ...
            if (list == null || reader.readerPool != null) {
                list = (List<T>)CreateInstance(reader.readerPool);
                list.Clear();
        ...
        public override void Copy(List<T> src, ref List<T> dst) {
            int dstCount = 0;
            if (dst == null) {
