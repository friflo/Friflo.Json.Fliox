...
namespace Friflo.Json.Fliox.Schema.JSON
{
    ...
    /// from a set of given <see cref="JSONSchemaTemp"/>'s.<br/>
    /// The utility method <see cref="JsonTypeSchema.ReadSchemas"/> can be used to read a set of
    /// <see cref="JSONSchemaTemp"/>'s as files in a folder.
    /// </summary>
    public sealed class JsonTypeSchema : TypeSchema, IDisposable
    {
        ...
        public JsonTypeSchema(List<JSONSchemaTemp> schemaList, string rootType = null) {
            typeMap = new Dictionary<string, JsonTypeDef>();
            foreach (JSONSchemaTemp schema in schemaList) {
        ...
        private static string GetNamespace (JSONSchemaTemp schema, string typeName) {
        ...
        /// <summary>Read a set of <see cref="JSONSchemaTemp"/>'s stored as files in the given <paramref name="folder"/>.</summary>
        public static List<JSONSchemaTemp> ReadSchemas(string folder) {
            string[] fileNames = Directory.GetFiles(folder, "*.json", SearchOption.TopDirectoryOnly);
            var schemas = new List<JSONSchemaTemp>();
            using (var typeStore    = new TypeStore())
            using (var reader       = new ObjectReader(typeStore)) {
                foreach (var path in fileNames) {
                    ...
                    var schema = reader.Read<JSONSchemaTemp>(jsonSchema);
    ...
    {
        internal readonly   JSONSchemaTemp                      schema;
