        ...
        public TypeMapper MatchTypeMapper(Type type, StoreConfig config) {
            ...
            if (type.IsClass || type.IsValueType || notInstantiatable) {
                ...
                if (notInstantiatable && factory == null)
                    throw new InvalidOperationException($"type requires instantiatable types by [FloInstance()] or [FloPolymorph()] on: {type}");
            ...
            if (factory != null) {
                ...
                if (discriminator == null) {
                    ...
                    if (ev == JsonEvent.ValueString && reader.parser.key.IsEqualString(discriminator)) {
                        ...
                        if (classType.IsNull(ref obj))
                            return reader.ErrorMsg<TypeMapper>($"No [FloPolymorph] type declared for discriminant: '{discriminant}' on type: ", classType.type.FullName, out success);
                        classType = reader.typeCache.GetTypeMapper(obj.GetType());
                        parser.NextEvent();
                    } else
