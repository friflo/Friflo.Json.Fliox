...
namespace Friflo.Json.Fliox.Hub.Remote.Transport.Udp
{
    ...
    public sealed class UdpSocketClientHub : SocketClientHub
    {
        ...
        private async Task ReceiveMessageLoop(ObjectReader reader) {
            var bufferSegment = udp.socket != null ? new ArraySegment<byte>(new byte[0x10000]) : default;
            while (true)
            {
                try {
                    // --- read complete datagram message
                    var result  = await udp.socket.ReceiveFromAsync(bufferSegment, SocketFlags.None, remoteHost).ConfigureAwait(false);
                    ...
                    if (env.logMessages) TransportUtils.LogMessage(Logger, sbRecv, $"c:{localPort,5} <-", remoteHost, message);
        ...
        public override async Task<ExecuteSyncResult> ExecuteRequestAsync(SyncRequest syncRequest, SyncContext syncContext) {
            ...
            try {
                ...
                using (var pooledMapper = syncContext.ObjectMapper.Get()) {
                    ...
                    if (env.logMessages) TransportUtils.LogMessage(Logger, sbSend, $"c:{localPort,5} ->", remoteHost, rawRequest);
                    // --- Send message
