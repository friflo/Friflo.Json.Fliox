    ...
    public struct ValueParser : IDisposable
    {
		...
		public int ParseInt(ref Bytes bytes, ref ValueError valueError, out bool success) {
			...
			for (int n = first; n < bytes.end; n++)
			{
				int digit = str[n] - '0';
				if (digit < 0 || digit > 9) {
					valueError.SetErrorFalse ($"Invalid character when parsing integer: '{bytes.ToFixed32()}'");
					return 0;
				}
				if (val < multLimit) {
					valueError.SetErrorFalse ($"Value out of range when parsing integer: '{bytes.ToFixed32()}'");
					return 0;
				}
				val *= 10;
				if (val < limit + digit) {
		...
		public long ParseLong(ref Bytes bytes, ref ValueError valueError, out bool success) {
			...
			for (int n = first; n < bytes.end; n++)
			{
				int digit = str[n] - '0';
				if (digit < 0 || digit > 9) {
					valueError.SetErrorFalse ($"Invalid character when parsing long: '{bytes.ToFixed32()}'");
					return 0;
				}
				if (val < multLimit) {
					valueError.SetErrorFalse ($"Value out of range when parsing long: '{bytes.ToFixed32()}'");
					return 0;
				}
				val *= 10;
				if (val < limit + digit) {
		...
		public double ParseDouble(ref Bytes bytes, ref ValueError valueError, out bool success)
		{
			...
			int n = bytes.start;
			if (n >= end) {
				valueError.SetErrorFalse($"Invalid number: '{bytes.ToFixed32()}'");
				return 0;
			}
			...
			for (; n < end; n++)
			{
				...
				case '.':
					if (comma == -1)
						...
						valueError.SetErrorFalse($"Invalid floating point number: '{bytes.ToFixed32()}'");
						return 0;
					}
		...
		public double ParseDoubleStd(ref Bytes bytes, ref ValueError valueError, out bool success) {
			...
			if (double.TryParse(val, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out double result))
				return result;
			success = false;
			valueError.SetErrorFalse ($"Parsing double failed. val: '{bytes.ToFixed32()}'");
			return 0;
		}
    }
