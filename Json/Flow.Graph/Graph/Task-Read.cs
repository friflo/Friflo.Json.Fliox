...
namespace Friflo.Json.Flow.Graph
{
    ...
    public class ReadTask<T> : SyncTask, IReadRefsTask<T> where T : class
    {
        ...
        // --- Ref
        public ReadRefTask<TRef> ReadRef<TKey2, TRef>(Expression<Func<T, Ref<TKey2, TRef>>> selector) where TRef : class {
            if (State.IsSynced())
                throw AlreadySyncedError();
        ...
        public ReadRefTask<TRef> ReadRefPath<TKey2, TRef>(RefPath<TKey2, T, TRef> selector) where TRef : class {
            if (State.IsSynced())
                throw AlreadySyncedError();
