...
namespace Friflo.Json.Flow.Graph
{
    ...
    public class Find<T> : FindTask<T> where T : Entity
    {
        ...
        internal override   string      Label       => name ?? $"Find<{typeof(T).Name}> (id: '{id}')";
    ...
    public class FindRange<T> : FindTask<T> where T : Entity
    {
        ...
        internal override   TaskState       State       => findState;
        internal override   string          Label       => name ?? $"FindRange<{typeof(T).Name}> (#ids: {ids.Count})";
        public   override   string          ToString()  => Label;
    ...
    public class ReadTask<T> : SyncTask, IReadRefsTask<T> where T : Entity
    {
        ...
        internal override   string                  Label       => name ?? $"ReadTask<{typeof(T).Name}> (#ids: {results.Count})";
        public   override   string                  ToString()  => Label;
