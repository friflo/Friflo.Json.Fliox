...
namespace Friflo.Json.Fliox.DB.Client
{
    ...
    public class EntityStore : ITracerContext, IDisposable
    {
        ...
        /// </summary>
        public EntityStore(EntityDatabase database, TypeStore typeStore, string userId, string clientId)
        {
        ...
        // --- SynchronizeAsync / TrySynchronizeAsync
        ...
        private SyncRequest CreateSyncRequest(out SyncStore syncStore) {
            ...
            // --- create new SyncStore and SyncSet's to collect future SyncTask's and executed via the next SynchronizeAsync() 
            foreach (var setPair in _intern.setByType) {
                EntitySet set = setPair.Value;
                set.ResetSync();
            }
