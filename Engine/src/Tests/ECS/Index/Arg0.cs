...
{
    /*
    private static void QueryArg01 (IndexContext cx)
    {
        var store = cx.store;
        ...
        var query5  = store.Query().                  HasValue<AttackComponent, Entity>(cx.target);
        cx.query1 = query1;
        cx.query2 = query2;
        cx.query3 = query3;
        {
            int count = 0;
            query1.ForEachEntity((Entity entity) => {
                switch (count++) {
            ...
            int count = 0;
            query2.ForEachEntity((Entity entity) => {
                switch (count++) {
            ...
            var count = 0;
            query3.ForEachEntity((Entity entity) => {
                switch (count++) {
                    case 0: AreEqual(11, entity.Id); break;
                    case 1: AreEqual(13, entity.Id); break;
            ...
            var count = 0;
            query5.ForEachEntity((Entity entity) => {
                count++;
