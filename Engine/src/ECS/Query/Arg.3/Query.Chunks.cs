...
public readonly struct QueryChunks<T1, T2, T3>  : IEnumerable <(Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities)>
...
[ExcludeFromCodeCoverage]
public struct ChunkEnumerator<T1, T2, T3> : IEnumerator<(Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities)>
    where T1 : struct, IComponent
    where T2 : struct, IComponent
    where T3 : struct, IComponent
{
    private readonly    ArchetypeQuery<T1, T2, T3>                          query;  //  8
    private readonly    (Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities)[]  chunks; //  8
    ...
    {
        this.query      = query;
        var chunkArrays = query.chunkArrays ?? new Stack<(Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities)[]>();
        ...
        chunks ??= new (Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities)[chunkCount];
        ...
        for (int n = 0; n < archetypes.length; n++)
        {
            ...
            for (int chunkPos = 0; chunkPos <= chunkEnd; chunkPos++)
            {
                ...
                chunks[pos++]       = new ValueTuple<Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities>(chunk1, chunk2, chunk3, entities);
            }
        }
        last    = pos - 1;
        index   = -1;
    ...
    // --- IEnumerator<>
    public readonly (Chunk<T1>, Chunk<T2>, Chunk<T3>, ChunkEntities) Current => chunks[index];
